...
export function LookGenerator() {
  ...
  async function handleSubmit(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      router.push(`/looks/${payload.lookId}`);
    } catch {
      setError(
    ...
    <section className="overflow-hidden rounded-[2rem] border border-slate-200 bg-white shadow-[0_24px_70px_-42px_rgba(15,23,42,0.55)]">
      ...
      <form className="p-6 sm:p-8" onSubmit={handleSubmit}>
        ...
        <div className="mt-6 space-y-5 rounded-2xl bg-slate-100 p-5 sm:p-6">
          <p className="font-mono text-[0.68rem] font-semibold uppercase tracking-[0.14em] text-slate-600">
            Model measurements
          </p>
          <label className="block text-sm font-medium text-slate-800">
            ...
            <input
              className="mt-2 block w-full rounded-xl border border-slate-500 bg-white px-3 py-2.5 text-slate-950 outline-none transition focus-visible:border-cyan-700 focus-visible:ring-4 focus-visible:ring-cyan-700/30"
              min="1"
              name="bodyChestCm"
                        ...
                        ({ key, label, placeholder }) => (
                          <td className="px-1.5 py-1.5" key={key}>
                            <input
                              ...
                              className="block w-full min-w-0 rounded-lg border border-slate-500 px-2 py-2 text-sm font-medium text-slate-950 outline-none transition focus-visible:border-cyan-700 focus-visible:ring-4 focus-visible:ring-cyan-700/30"
                              min="1"
                              onChange={(event) =>
        ...
        <div className="mt-7 flex flex-col gap-4 border-t border-slate-200 pt-6 sm:flex-row sm:items-center sm:justify-between">
          ...
          <button
            className="rounded-full bg-cyan-700 px-5 py-3 text-sm font-semibold text-white transition hover:bg-cyan-800 focus-visible:outline-none focus-visible:ring-4 focus-visible:ring-cyan-300 disabled:cursor-wait disabled:bg-slate-500"
            disabled={isSubmitting}
            type="submit"
          >
...
function ImageUrlField({
  ...
  return (
    <label className="block text-sm font-medium text-slate-800">
      ...
      <input
        className="block w-full rounded-xl border border-slate-500 bg-white px-3 py-2.5 text-slate-950 outline-none transition focus-visible:border-cyan-700 focus-visible:ring-4 focus-visible:ring-cyan-700/30"
        inputMode="url"
        onChange={(event) => onChange(event.target.value)}
        placeholder="https://…"
        required
        ...
        value={value}
      />
      <span className="mt-3 flex aspect-[4/3] items-center justify-center overflow-hidden rounded-2xl bg-slate-100 text-xs font-medium uppercase tracking-[0.16em] text-slate-600">
        {preview ? (
          <img
            alt={`${label} preview`}
