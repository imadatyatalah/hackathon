...
export const looks = pgTable(
  ...
  (table) => [
    index("looks_userId_idx").on(table.userId),
    uniqueIndex("looks_providerRequestId_uidx").on(table.providerRequestId),
  ],
);
...
export const looksRelations = relations(looks, ({ one }) => ({
  user: one(user, {
    fields: [looks.userId],
    references: [user.id],
  }),
